...
namespace odfaeg {
    namespace window {
        ...
        void Win32Window::create (VideoMode mode, const String& title, Uint32 style) {
            // Set that this process is DPI aware and can handle DPI scaling
            setProcessDpiAware();
        ...
        IKeyboard::Key Win32Window::virtualKeyCodeToODFAEG(WPARAM key, LPARAM flags)
        {
            switch (key)
            {
                ...
                case VK_CONTROL : return (HIWORD(flags) & KF_EXTENDED) ? odfaeg::window::IKeyboard::RControl : Keyboard::LControl;

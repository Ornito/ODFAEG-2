...
namespace odfaeg {
    namespace graphic {
        namespace ecs {
            ...
            bool PerPixelLinkedListRenderComponent::loadEntitiesOnComponent(std::vector<EntityId> vEntities) {
                batcher.clear();
                normalBatcher.clear();
                    ...
                    if (getComponent<MeshComponent>(vEntities[i]) != nullptr) {
                        ...
                        for (unsigned int j = 0; j <  getComponent<MeshComponent>(vEntities[i])->faces.size(); j++) {
                             if (getComponent<EntityInfoComponent>(vEntities[i])->drawMode == INSTANCED) {
                                ...
                                /*for (unsigned int k = 0; k < getComponent<MeshComponent>(vEntities[i])->faces[j].getVertexArray().getVertexCount(); k++) {
                                    std::cout<<"vertex pos : "<<getComponent<MeshComponent>(vEntities[i])->faces[j].getVertexArray()[j].position.x<<std::endl;
                                }*/
                                //std::cout<<"transform matrix : "<<getComponent<MeshComponent>(vEntities[i])->faces[j].getTransformMatrix().getMatrix()<<std::endl;
                                normalBatcher.addFace(&getComponent<MeshComponent>(vEntities[i])->faces[j]);
                             }
                        }
                    }

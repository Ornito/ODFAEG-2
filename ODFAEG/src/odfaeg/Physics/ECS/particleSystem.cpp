#include "../../../../include/odfaeg/Physics/ECS/particleSystem.hpp"
/////////////////////////////////////////////////////////////////////////////////
//
// Thor C++ Library
...
namespace odfaeg
{
    namespace physic {
        ...
        // ---------------------------------------------------------------------------------------------------------------------------
            ...
            ParticleSystem::ParticleSystem(math::Vec3f position, math::Vec3f size, graphic::ecs::EntityFactory& factory)
            : mParticles()
            , mAffectors()
            , mEmitters()
            ...
            {
                entity = factory.createEntity("E_PARTICLES");
                ...
                graphic::VertexArray va(sf::Quads);
                va.setEntityId(entity);
                graphic::Face face (va, material, tm);
                graphic::ecs::MeshComponent mc;
                mc.faces.push_back(face);
                graphic::ecs::addComponent(entity, eic);
                graphic::ecs::addComponent(entity, tc);
                graphic::ecs::addComponent(entity, mc);
            }
            void ParticleSystem::setTexture(const graphic::Texture& texture)
            {
                graphic::ecs::getComponent<graphic::ecs::MeshComponent>(entity)->faces[0].getMaterial().clearTextures();
                graphic::ecs::getComponent<graphic::ecs::MeshComponent>(entity)->faces[0].getMaterial().addTexture(&texture);
                mNeedsQuadUpdate = true;
            }

            unsigned int ParticleSystem::addTextureRect(const sf::IntRect& textureRect)
            {
                graphic::ecs::getComponent<graphic::ecs::MeshComponent>(entity)->faces[0].getMaterial().addTexture(nullptr, textureRect);

                return graphic::ecs::getComponent<graphic::ecs::MeshComponent>(entity)->faces[0].getMaterial().getNbTextures() - 2;
            }
            ...
            void ParticleSystem::draw(graphic::RenderTarget& target, graphic::RenderStates states)
            {
                ...
                states.texture = graphic::ecs::getComponent<graphic::ecs::MeshComponent>(entity)->faces[0].getMaterial().getTexture();
                target.draw(graphic::ecs::getComponent<graphic::ecs::MeshComponent>(entity)->faces[0].getVertexArray(), states);
            }
            ...
            void ParticleSystem::computeVertices() const
            {
                ...
                graphic::VertexArray& va = graphic::ecs::getComponent<graphic::ecs::MeshComponent>(entity)->faces[0].getVertexArray();
                va.clear();
                // Fill vertex array
                ...
                for(it = mParticles.begin(); it < mParticles.end(); it++)
                {
                    ...
                    assert(it->textureIndex == 0 || it->textureIndex < graphic::ecs::getComponent<graphic::ecs::MeshComponent>(entity)->faces[0].getMaterial().getNbTextures()-1);
            ...
            void ParticleSystem::computeQuads() const
            {
                ...
                assert(graphic::ecs::getComponent<graphic::ecs::MeshComponent>(entity)->faces[0].getMaterial().getTexture());

                // No texture rects: Use full texture, cache single rectangle
                if (graphic::ecs::getComponent<graphic::ecs::MeshComponent>(entity)->faces[0].getMaterial().getNbTextures() < 2)
                {
                    mQuads.resize(1);
                    computeQuad(mQuads[0], getFullRect(*graphic::ecs::getComponent<graphic::ecs::MeshComponent>(entity)->faces[0].getMaterial().getTexture()));
                }

                // Specified texture rects: Cache every one
                else
                {
                    mQuads.resize(graphic::ecs::getComponent<graphic::ecs::MeshComponent>(entity)->faces[0].getMaterial().getNbTextures()-1);
                    for (std::size_t i = 0; i < graphic::ecs::getComponent<graphic::ecs::MeshComponent>(entity)->faces[0].getMaterial().getNbTextures()-1; ++i)
                        computeQuad(mQuads[i], graphic::ecs::getComponent<graphic::ecs::MeshComponent>(entity)->faces[0].getMaterial().getTexRect(i+1));
                }
            }

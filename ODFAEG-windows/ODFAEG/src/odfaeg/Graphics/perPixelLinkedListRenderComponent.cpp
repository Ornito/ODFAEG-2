                ...
                for (unsigned int i = 0; i < m_normals.size(); i++) {
                   if (m_normals[i].getAllVertices().getVertexCount() > 0) {
                        ...
                        for (unsigned int j = 0; j < m_normals[i].getAllVertices().getVertexCount(); j++) {
                            if ( m_normals[i].getVertexArrays()[0]->getEntity()->getRootType() == "E_WALL")
                                std::cout<<"type : "<<m_normals[i].getVertexArrays()[0]->getEntity()->getRootType()<<"position : "<<m_normals[i].getAllVertices()[j].position.x<<","<<m_normals[i].getAllVertices()[j].position.y<<std::endl;
